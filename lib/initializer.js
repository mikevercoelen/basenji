...
function buildDeps (packageDeps, targetDeps) {
  var output = '';
  var targetDepsLength = targetDeps.length;
  var i = 1;

  for (var d in packageDeps) {
    var version = packageDeps[d]

    if (targetDeps.indexOf(d) > -1) {
      output += '"' + d + '": "' + version + '"';

      if (i !== targetDepsLength) {
        output += ', \n\t\t';
      }
...
getGitRepoUrl(function (error, gitRepoUrl) {
  inquirer.prompt(prompts).then(function (answers) {
    ...
    var packageDeps = package.dependencies;

    answers.devDependencies = buildDeps(packageDeps, [
      'babel-core',
      'babel-eslint',
    ...
    var dependencies = buildDeps(packageDeps, [
      'basenji-tools'
    ]);
